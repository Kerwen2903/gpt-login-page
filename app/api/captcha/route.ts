...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      console.error("[v0] Captcha API error:", response.status, response.statusText)
      return NextResponse.json({ error: `Failed to fetch captcha: ${response.status}` }, { status: response.status })
    }
    ...
    if (!captchaId) {
      return NextResponse.json({ error: "Captcha ID not found in response headers" }, { status: 500 })
    }
