...
export function LoginForm() {
  ...
  const loadCaptcha = async () => {
    try {
      ...
      setError(`Failed to load captcha: ${error instanceof Error ? error.message : "Unknown error"}`)
    }
  }
  ...
  return (
    <Card className="w-full shadow-lg">
      <CardHeader className="text-center space-y-2">
        ...
        <CardTitle className="text-2xl font-bold text-balance">Login to Your GPT Account</CardTitle>
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1 bg-muted p-2 rounded-md border flex items-center justify-center min-h-[60px]">
                {captchaImageUrl ? (
                  ...
                  <div className="text-muted-foreground text-sm">Loading captcha...</div>
                )}
              </div>
              <Button
                type="button"
